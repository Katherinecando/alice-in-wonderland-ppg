...
up.paths.root_folder ='C:\Users\katyl\OneDrive - City, University of London\year 3\individual project\RRest_data\';
...
up.al.options.extract_resp_sig = {'ppg_filt'};  % Possible methods: 'ppg_feat', 'ekg_feat', 'ppg_filt', 'ekg_filt'
% ------------------------------------------------------
...
up.al.options.ppg_filt = {'BFi'};                             % Possible methods: 'Wfm', 'Wam', 'CCF', 'BFi'
...
up.al.options.estimate_rr = {'WCH'};   % Welch spectral method
...
% disp("DEBUG root_folder = " + up.paths.root_folder)
% disp("DEBUG data_load_folder = " + up.paths.data_load_folder)
% disp("DEBUG data_path = " + data_path)
...
% end
% --- No-toolbox replacement for extractfield
up.paramSet.groups = arrayfun(@(s) s.group, data);

if isfield(data, 'sub_group')
    up.paramSet.sub_groups = arrayfun(@(s) s.sub_group, data);
end
