...
%  Author: Katherine  |  EG3000 Dissertation 2025â26
% =========================================================================

%% ââ SECTION 1: What is synchrony? ââââââââââââââââââââââââââââââââââââââââ
%
...
%
%        S(t) = (2 / N(N-1)) * Î£_{i<j} r_ij(t)
%
%  i.e., the mean of all pairwise Pearson correlations in that window.
%  S(t) â [-1, 1], where:
%       S â 0  â no synchrony (random phases)
%       S â 1  â all participants' signals are perfectly in phase
%       S â -1 â anti-phase (unlikely in practice)

N_demo = 5;   % toy example with 5 participants
fprintf('For N = %d participants:\n', N_demo);
fprintf('  Unique pairs = N*(N-1)/2 = %d\n', N_demo*(N_demo-1)/2);

%% ââ SECTION 2: Pearson correlation reminder âââââââââââââââââââââââââââââ
%
%  For two time-series x and y over window W of length L:
%
%        r(x,y) = cov(x,y) / (std(x) * std(y))
%               = Î£[(x_k - xÌ)(y_k - È³)] / sqrt(Î£(x_k-xÌ)Â² * Î£(y_k-È³)Â²)
%
...
r_AC = corr(x, z);
fprintf('\nDemo correlations:\n');
fprintf('  r(A,B) = %.3f  (similar traces â high r)\n', r_AB);
fprintf('  r(A,C) = %.3f  (independent   â low  r)\n', r_AC);

%% ââ SECTION 3: Full pairwise correlation matrix âââââââââââââââââââââââââ
%
%  For a segment matrix SEG of size [L Ã N] (L time-points, N participants):
%
%        R = corr(SEG, 'Rows','pairwise')    % [N Ã N] symmetric matrix
%
...
%% ââ SECTION 4: Sliding window parameters used in this study âââââââââââââ
%
%  PULSE RATE synchrony   (peaks_final_analysis.m)
%    Signal:    H  [4801 Ã N]  pulse rate at 1 Hz, t = 0..4800 s
%    Window:    W  = 120 s   (captures ~2 respiratory cycles of PR fluctuation)
%    Step:      Î´t = 1  s    (1-second resolution output)
...
%    Signal:    RR [1201 Ã N]  respiratory rate at 0.25 Hz, t = 0..4800 s
%    Window:    W  = 60  s
%    Step:      Î´t = 5   s
%    QC:        subjects with >30% NaN on common grid excluded
%               windows require â¥70% valid samples per participant
...
%% ââ SECTION 5: Reproduce the sliding window loop (pseudocode + real) âââââ
%
...
%    for k = 1 : numel(t_mid)
%        idx0 = (k-1)*step + 1;
%        seg  = H2(idx0 : idx0+win-1, :);          % [win Ã N] segment
%
%        v    = var(seg, 0, 1);
...
%    end
%
%  Output: sync(k) = S(t_k)   â the synchrony time-series plotted in results

disp('Section 5: see inline comments above for the full loop.');

%% ââ SECTION 6: Statistical comparisons ââââââââââââââââââââââââââââââââââ
%
...
%    4. Extract r_ij values for each window â distributions peak_r, base_r
...
%    Also: cross-correlation with Â±50-step lag test to detect temporal offset
