...
%   t_mid   — time vector for HR synchrony windows (seconds)
%   sync_s  — smoothed HR synchrony timecourse
%
% REQUIRES on disk (in working directory):
%   breathing_synchrony_results.mat  — output from plot_rr_overlay.m

% Load breathing synchrony from RRest pipeline (RR_final.mat → breathing_synchrony_analysis.m)
rrest_alg_path = fullfile('..', 'RRest-master', 'RRest-master', 'RRest_v3.0', 'Algorithms');
B = load(fullfile(rrest_alg_path, 'breathing_synchrony_results.mat'));
outB = B.out;
...
title('Breathing vs PR synchrony (same window/step)');
legend({'Breathing','PR'}, 'Location','best');
ylim([-0.2 1]);

% Check around 65 min
target_min = 65;
[~, idx65] = min(abs(t_sync_b/60 - target_min));
xline(t_sync_b(idx65)/60, '--', '65 min', 'HandleVisibility', 'off');
...
fprintf('\nBreathing–HR synchrony correlation = %.3f\n', r_system);
