...
% - Plots all participants + audience mean ± SD (1 Hz resample)
clear; clc;
...
ylabel("HRM (bpm)");
title(sprintf("All sessions: HRM points (0–%d min performance window)", perfMinutes));
xlim([0 perfMinutes]);

%% ---------- PLOT 2: audience mean ± SD (resampled to 1 Hz) ----------
...
xlabel("Time from start (min)");
ylabel("HRM (bpm)");
title(sprintf("Audience mean HRM ± SD (1 Hz resample, 0–%d min)", perfMinutes));
xlim([0 perfMinutes]);
